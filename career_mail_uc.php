...
$body = '
	<table width="100%" border="0" cellpadding="0" style="font:12px Arial, serif;color:#222;">
	  ...
	  <tr>
		<td><p><span style="color:#0065B3; font-size:14px; font-weight:bold">Application mail</span><br />
...
$record->current_address = !empty($_REQUEST['address'] )?$_REQUEST['address']:'';
$record->mobile 	= !empty($_REQUEST['phone'])?$_REQUEST['mobile']:'';
$record->email 		= $_REQUEST['email'];
// $record->position 	= !empty($_REQUEST['position'])?$_REQUEST['position']:'';
$record->myfile 	= $_REQUEST['fileArrayname'];
$record->qualification = !empty($_REQUEST['message'])?$_REQUEST['qualification']:'';
