...
class team extends DatabaseObject
{
    ...
    protected static $table_name = "tbl_team";
    protected static $db_fields = array('id', 'name', 'title','image', 'facebook', 'instagram', 'social', 'email','phone', 'content','role', 'status', 'sortorder', 'added_date');
    ...
    public static function find_all_by_role($role = '')
    {
        global $db;
        $add = (!empty($role)) ? " AND role = '{$role}'" : '';
        return self::find_by_sql("SELECT * FROM " . self::$table_name . " WHERE status=1 " . $add . " ORDER BY sortorder DESC");
    }
