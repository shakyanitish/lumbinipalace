...
class Module extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'parent_id', 'name', 'link', 'mode', 'icon_link', 'status', 'sortorder', 'added_date', 'properties','upcoming');
	...
	//Find all the rows in the current database table.
	public static function find_all_parent_dash(){
	global $db;
	return self::find_by_sql("
		SELECT t.* 
		FROM " . self::$table_name . " AS t
		LEFT JOIN " . self::$table_name . " AS p 
			ON (t.parent_id = 0 AND t.id = p.id) 
			OR (t.parent_id != 0 AND t.parent_id = p.id)
		WHERE 
		  t.status = '1' AND
		  p.status = '1' AND
		  (
			(t.parent_id = 0 AND t.id NOT IN (
				SELECT DISTINCT parent_id 
				FROM " . self::$table_name . " 
				WHERE parent_id != 0
			))
			OR
			(t.parent_id != 0 AND t.parent_id IN (
	...
	public static function find_all_parent_upcoming(){
		global $db;
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE status='1' AND parent_id= 0 AND upcoming= 1 ORDER BY sortorder ASC");
	}
