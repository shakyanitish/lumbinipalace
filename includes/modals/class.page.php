...
class Page extends DatabaseObject {
	...
	// Homepage Display
	public static function homepagePage($limit='') {
		global $db;
	...
	public static function find_all_active_uc(){
        global $db;
        return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE status=1 AND upcoming=1 ORDER BY sortorder DESC");
    }
	...
	public function save(){
		return isset($this->id) ? $this->update() : $this->create();// If the object has an id, it already exists in the database, so call update. Otherwise, call create to add a new record.
	}
	...
	public function create(){
		...
		$sql.= ") VALUES ('";
		$sql.= join("', '", array_values($attributes));// Join the attribute values with "', '"
		$sql.= "')";
		if($db->query($sql)){
	...
	public function update(){
		global $db;
		//sanitize means to make safe for database to use
		$attributes = $this->sanitized_attributes();
		$attribute_pairs = array();// key value pair array initialize
		...
		// Join the attribute pairs with a comma and space (", ") to form the SET clause
		$sql.= join(", ", array_values($attribute_pairs));
		$sql.= " WHERE id=".$db->escape_value($this->id);
		$db->query($sql);// Execute the update query
...
public function getImages() {
    if (!empty($this->image)) {
        return unserialize($this->image);// Unserialize = convert the gallery images string to an array
    }
    return [];
}

// Add a new image in the image array
public function addImage($filename) {
    $images = $this->getImages();
    $images[] = $filename;
    $this->image = unserialize($images);
}
