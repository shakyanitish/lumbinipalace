...
class Article extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'parent_id', 'slug', 'title', 'sub_title', 'linksrc', 'linktype', 'content', 'upcoming', 'status', 'sortorder', 'meta_title', 'meta_keywords', 'meta_description', 'type', 'added_date', 'modified_date','homepage','brief', 'image');
	...
	public static function get_by_Parent($parent=0){
		global $db;
		$sql = "SELECT id, slug, title, linksrc, linktype,brief, content, image FROM ".self::$table_name." WHERE status=1 AND parent_id='".$parent."' ORDER BY sortorder ASC";
