...
class VirtualTour {
	//DATABASE TABLE NAME
    protected static $table_name = "tbl_vt_virtual_tour";
	//DATABASE TABLE FIELDS
	protected static $db_fields = array('id','title','status','sortorder','added_date','scene_fade_duration','image_width','image_height','hotspot_icon');
    //PROPERTIES OF VIRTUAL TOUR
	public $id, $title, $scene_id, $status, $sortorder, $added_date, $scene_fade_duration, $image_width ,$image_height, $hotspot_icon;
    ...
    public static function find_by_id_active($id=0){
        global $db;
        $result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE status=1 AND id={$id} LIMIT 1");
        // $sql = "SELECT * FROM ".self::$table_name." WHERE subpackageid={$id} LIMIT 1";
        // pr($sql);
        return !empty($result_array) ? array_shift($result_array) : false;
    }
	...
	//GET VIRTUAL TOUR NAME BY IT'S ID
	static function getVirtualTour($id=0){
		global $db;
