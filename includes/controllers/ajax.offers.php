...
switch ($action) {
	case "add":
		...
		$record->deadline_type = $_REQUEST['deadline_type'];
		
		// Only save dates if deadline_type is "deadline", otherwise clear them
		if ($_REQUEST['deadline_type'] == 'deadline') {
		...
		if ($record->save()):
			...
			$db->rollback();
			echo json_encode(array("action" => "error", "message" => $GLOBALS['basic']['unableToSave']));		endif;		break;
	case "edit":
