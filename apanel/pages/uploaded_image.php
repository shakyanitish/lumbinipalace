...
if(!isset($_SESSION['imageNameArr']) || !is_array($_SESSION['imageNameArr'])) {
    $_SESSION['imageNameArr'] = [];
}
...
<div class="col-md-3" id="previewUserimage<?php echo $deleteid;?>">
    <div class="infobox info-bg">
        <img src="<?php echo IMAGE_PATH.'pages/thumbnails/'.$val;?>"  style="width:100%"/>
...
//uplodify
  if(isset($_SESSION['imageNameArr'])){
   if(count($_SESSION['imageNameArr'])>0){
      ...
      { 
         @unlink(IMAGE_PATH.'pages/thumbnails/'.$val);// delete thumbnail
      	 @unlink(IMAGE_PATH.'pages/'.$val);// delete original
      }	 
    	unset($_SESSION['imageNameArr']);//unset removes the session variable completerly
       }
  }
//uplodify
?>
