        ...
        <div class="form-row">
            ...
            <div class="col-md-3" id="previewUserimage<?php echo $deleteid; ?>">
                <div class="infobox info-bg">
                    <img src="<?php echo IMAGE_PATH . 'product/thumb/thumbnails/' . $val; ?>" style="width:100%"/>
                    ...
                    <input type="hidden" name="imageThumb" value="<?php echo $val; ?>" class="validate[required,length[0,250]]"/>
                </div>
            </div>
        </div>
...
if (isset($_SESSION['imageNameArr'])) {
    if (count($_SESSION['imageNameArr']) > 0) {
        foreach ($_SESSION['imageNameArr'] as $key => $val) {
            @unlink(IMAGE_PATH . 'product/thumb/thumbnails/' . $val);
            @unlink(IMAGE_PATH . 'product/thumb/' . $val);
        }
