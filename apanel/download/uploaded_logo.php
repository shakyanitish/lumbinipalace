...
<div class="col-md-3" id="previewUserimage<?php echo $deleteid;?>">
    <div class="infobox info-bg">
        <img src="<?php echo IMAGE_PATH.'download/thumbnails/'.$val;?>"  style="width:100%"/>
        ...
        <input type="hidden" name="imageArraylogo" value="<?php echo $val;?>" class="validate[required,length[0,250]]" />        
    </div> 
</div>
</div>
...
//uplodify
  if(isset($_SESSION['imageNameArr'])){
   if(count($_SESSION['imageNameArr'])>0){
      ...
      { 
         @unlink(IMAGE_PATH.'download/thumbnails/'.$val);
      	 @unlink(IMAGE_PATH.'download/'.$val);
      }	 
